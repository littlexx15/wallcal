...
打开 **「显示设置」**，选择 **自动避让图标** 或 **全屏**。自动布局读取 Windows 主桌面的图标位置，在可用的连续空白区域放置日历，无需手动计算桌面占比，也不需要隐藏图标。
...
第三方桌面整理面板暂不支持自动识别。没有足够空白区域或读取失败时，会保留当前壁纸并提示；目前尚不支持多显示器独立日历布局。
...
2. 下载 `WallCal-1.5.5-Windows.zip`，解压并阅读包内运行须知。
...
输出 `dist\WallCal-1.5.5.exe` 和 `dist\WallCal-1.5.5-Windows.zip`，压缩包自动附带运行须知与卸载恢复说明。
